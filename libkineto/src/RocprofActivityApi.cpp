...
int RocprofActivityApi::processActivities(
  ...
  // Process all external correlations pairs. stopLogging() drains the
  // rocprofiler buffer before this runs, but it gives up after a bounded
  // number of attempts, so a late append is still possible here. The write
  // path holds externalCorrelationsMutex_; take ownership under it rather
  // than iterating the container in place.
  ...
  // Same reasoning as the external correlations above: insert_row_to_buffer()
  // appends under rowsMutex_, and a concurrent push_back reallocates the
  // vector and invalidates both the backfill pass and the iteration below.
